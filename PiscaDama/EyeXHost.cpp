...
void EyeXHost::Init(HWND hWnd, UINT statusChangedMessage, UINT focusedRegionChangedMessage, UINT regionActivatedMessage, UINT gazeMessage)
{
	_hWnd = hWnd;
...
void EyeXHost::HandleActivatableEvent(TX_HANDLE hEvent, int interactorId)
{
	...
	TX_HANDLE hInteractable(TX_EMPTY_HANDLE);

	if (txGetEventBehavior(hEvent, &hInteractable, TX_INTERACTIONBEHAVIORTYPE_GAZEPOINTDATA) == TX_RESULT_OK)
	{
		OnGazeDataEvent(hInteractable, interactorId);
		txReleaseObject(&hInteractable);
	}
	else if (txGetEventBehavior(hEvent, &hActivatable, TX_INTERACTIONBEHAVIORTYPE_ACTIVATABLE) == TX_RESULT_OK)
	{
...
void EyeXHost::OnActivationFocusChanged(TX_HANDLE hBehavior, int interactorId)
{
	...
	if (txGetActivationFocusChangedEventParams(hBehavior, &eventData) == TX_RESULT_OK)
	{
		...
		else if (eventData.HasTentativeActivationFocus)
		{
			SetFocusedRegionId(interactorId);
			PostMessage(_hWnd, _gazeMessage,(WPARAM) interactorId, 0);
		}
		else
		{
 ...
 */
void EyeXHost::OnGazeDataEvent(TX_HANDLE hBehavior, int interactorId)
{
	TX_GAZEPOINTDATAEVENTPARAMS eventParams;

	if (txGetGazePointDataEventParams(hBehavior, &eventParams) == TX_RESULT_OK) {
		PostMessage(_hWnd, _gazeMessage,(WPARAM) interactorId, 0);
		//PostMessage(_hWnd, _gazeMessage,(WPARAM) eventParams.X, (LPARAM) eventParams.Y);
